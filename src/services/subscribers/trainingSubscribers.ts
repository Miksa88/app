...
export function registerTrainingSubscribers(): void {
  EventBus.subscribe('WORKOUT_COMPLETED', async (event) => {
    logger.info(`[Training] Workout completed: ${event.sessionId} ` +
  ...
  EventBus.subscribe('LEVEL_UP_ACHIEVED', async (event) => {
    logger.info(`[Training] Level up: ${event.clientId} → ${event.newLevel}`);
    // TODO Faza 4: celebration screen + push notif
  });

  EventBus.subscribe('LEVEL_DOWN_TRIGGERED', async (event) => {
    logger.warn(`[Training] Level down za ${event.clientId} — sugesti regresija`);
  });

  EventBus.subscribe('TRAINING_INTENSITY_REDUCE', async (event) => {
    logger.info(`[Training] Intensity reduce ${event.reduction * 100}% (${event.reason}) ` +
      `za ${event.clientId}`);
  });

  EventBus.subscribe('TRAINING_VOLUME_REDUCE', async (event) => {
    logger.info(`[Training] Volume reduce ${event.reduction * 100}% (${event.reason}) ` +
      `za ${event.clientId}`);
  });

  EventBus.subscribe('HYDRATION_FIRST_WARNING', async (event) => {
    logger.info(`[Training] Hydration first warning za ${event.clientId}: ${event.message}`);
