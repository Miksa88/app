...
describe('shouldStartDeload', () => {
  it('vraća shouldStart=true, reason=week_5_of_mesocycle kad je index poslednji', () => {
    const result = shouldStartDeload(4, 5, 'deficit');
    expect(result.shouldStart).toBe(true);
    expect(result.reason).toBe('week_5_of_mesocycle');
  });

  it('vraća shouldStart=false, reason=not_yet u prvoj nedelji novog mezo', () => {
    const result = shouldStartDeload(0, 5, 'deficit');
  ...
  it('lean_bulk preskače deload — shouldStart=false, reason=lean_bulk_no_deload', () => {
    const result = shouldStartDeload(4, 5, 'lean_bulk');
    expect(result.shouldStart).toBe(false);
...
describe('handleMesocycleEnd', () => {
  it('mid mezo (pointer < sessions.length): nema rollover, mesocycleJustEnded=false', () => {
    ...
    // pointer=0 je default iz builder-a, 20 sesija ukupno (4 dana × 5 nedelja)
    expect(queue.sessionPointer).toBe(0);
    expect(queue.sessions.length).toBe(20);

    const result = handleMesocycleEnd(queue, profile, skeleton, 5);
    expect(result.mesocycleJustEnded).toBe(false);
    expect(result.newQueue).toBe(queue); // referenca identična → nije novi queue
  });

  it('kraj mezo (pointer >= sessions.length): novi queue 20 sesija, poslednje 4 su isDeloadWeek=true', () => {
    const queue = buildBaseQueue();
    const exhausted: MesocycleQueue = {
      ...queue,
      sessionPointer: queue.sessions.length,  // 20 → end
    };

    const result = handleMesocycleEnd(exhausted, profile, skeleton, 5);
    expect(result.mesocycleJustEnded).toBe(true);
    expect(result.newQueue.sessions).toHaveLength(20);

    // Poslednja 4 sesije (indexi 16–19) su deload week (5. nedelja)
    const deloadSessions = result.newQueue.sessions.slice(16);
    expect(deloadSessions.every(s => s.isDeloadWeek === true)).toBe(true);

    // Prvih 16 sesija (4 load nedelje × 4 dana) NISU deload
    const nonDeloadSessions = result.newQueue.sessions.slice(0, 16);
...
describe('hasMesocycleEnded', () => {
  it('vraća false kad je pointer ispod length-a', () => {
    ...
    expect(hasMesocycleEnded({ ...queue, sessionPointer: 19 })).toBe(false);
  });
